...
class RealtimeBarrage {
  ...
  push(args, row) {
    ...
    args.forEach((item) => {
      ...
      let sprite = this._getSprite(item)
  ...
  _getSprite(item) {
    ...
    if (this.styleType === 's1') {
      ...
      if (item.isCurrUser) {
        styles = {
          preText: this._getStyle('red', {shadow: true}),
          afterText: this._getStyle('red', {shadow: true}),
        }
      } else {
        styles = {
          preText: this._getStyle('yellow', {shadow: true}),
          afterText: this._getStyle('yellow', {shadow: true}),
        }
      }
    ...
    // 允许扩展其他样式
    if (this.styleType === 's1') {
      return this._getSpriteS1(item, styles)
    }
  }

  // 构建一条弹幕的Sprite实例
  _getSpriteS1(item, styles) {
